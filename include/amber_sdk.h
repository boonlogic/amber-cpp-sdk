...
class amber_sdk {
public:
  explicit amber_sdk(const char *license_id = "default",
                     const char *license_file = "~/.Amber.license",
                     bool verify_cert = true, const char *cert = NULL,
  ...
  void set_cainfo(const char *cainfo) {
    ssl.cainfo = std::string(cainfo ? cainfo : "");
  }

  bool create_sensor(amber_models::create_sensor_response &response,
                     std::string *label = NULL);

  bool get_sensor(amber_models::get_sensor_response &response,
                  const std::string &sensor_id);

  bool list_sensors(amber_models::list_sensors_response &response);

  bool update_sensor(amber_models::update_sensor_response &response,
                     const std::string sensor_id, std::string &label);

  bool delete_sensor(const std::string &sensor_id);

  bool
  configure_sensor(amber_models::configure_sensor_response &response,
                   const std::string &sensor_id, uint16_t feature_count = 1,
  ...
  bool stream_sensor(amber_models::stream_sensor_response &response,
  ...
  bool get_pretrain(amber_models::get_pretrain_response &response,
                    const std::string &sensor_id);

  bool get_config(amber_models::get_config_response &response,
                  const std::string &sensor_id);

  bool get_status(amber_models::get_status_response &response,
                  const std::string &sensor_id);

  bool get_version(amber_models::get_version_response &response);

  bool get_root_cause_by_idlist(amber_models::get_root_cause_response &response,
                                const std::string &sensor_id,
                                std::string &id_list);

  bool
  get_root_cause_by_patternlist(amber_models::get_root_cause_response &response,
                                const std::string &sensor_id,
